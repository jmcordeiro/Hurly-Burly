...
version 0.5.1, December 23, 2010
...
¥ 0.5 as part of the timbreID-0.5 update, this gets rid of unnecessary getbytes(0) calls in timbreID.c. does not yet use memset() or memcpy() where possible.  fixed lack of memory free for x->weights, x->attribute_order, x->instance_feature_lengths, x->norm_data in _free.
...
static void timbreID_sort_knn_info(int k, int num_instances, int prev_match, t_knn_info *list)
{
	...
	top_matches = (int *)getbytes(k * sizeof(int));
...
static float timbreID_correlation(t_timbreID *x, float *v1, float *v2)
{
	...
	vec1Centered = (float *)getbytes(vecLen * sizeof(float));
	vec2Centered = (float *)getbytes(vecLen * sizeof(float));
...
static void timbreID_train(t_timbreID *x, t_symbol *s, int argc, t_atom *argv)
{
	...
	x->instances[instance_ind].instance = (float *)getbytes(list_length * sizeof(float));

	x->cluster_members[instance_ind].member = (int *)getbytes(2 * sizeof(int));
...
static void timbreID_id(t_timbreID *x, t_symbol *s, int argc, t_atom *argv)
{
	...
	if(x->num_instances)
	{
		votes = (int *)getbytes(x->num_instr * sizeof(int));
...
{	
	...
	// create local memory
	attribute_column = (float *)getbytes(x->num_instances * sizeof(float));
	...
	else
	{
		...
		{
			// create memory;
			x->norm_data = (t_norm_data *)getbytes(x->feature_length * sizeof(t_norm_data));
...
{
	int i, j, cluster_idx_i, low_i, hi_i, num_members, flag;
	...
	//	};
		...
		x->cluster_members[cluster_idx_i].member = (int *)getbytes(x->cluster_member_lengths[cluster_idx_i] * sizeof(int));		
...
static void timbreID_compute_cluster(t_timbreID *x, t_floatarg num_instr)
{
	...
	// create local memory
	min_dist_idx = (int *)getbytes(2 * sizeof(int));
	cluster_data = (t_instance *)getbytes(numInstances * sizeof(t_instance));
	pair_dists = (float *)getbytes(num_pairs * sizeof(float));
	listOut = (t_atom *)getbytes(numInstances * sizeof(t_atom));
	...
	for(i=0; i<numInstances; i++)
	{
		cluster_data[i].instance = (float *)getbytes(x->feature_length * sizeof(float));
...
static void timbreID_uncluster(t_timbreID *x)
{
	...
	for(i=0; i<x->num_instances; i++)
		x->cluster_members[i].member = (int *)getbytes(2 * sizeof(int));
...
static void timbreID_compute_variance(t_timbreID *x)
{
	...
	{
		// create local memory
		attribute_var = (float *)getbytes(x->feature_length * sizeof(float));
		meanCentered = (t_instance *)getbytes(x->num_instances * sizeof(t_instance));
	
		for(i=0; i<x->num_instances; i++)
			meanCentered[i].instance = (float *)getbytes(x->feature_length * sizeof(float));
...
static void timbreID_clusters_list(t_timbreID *x)
{
	...
	// create local memory
	listOut = (t_atom *)getbytes(x->num_instances * sizeof(t_atom));
...
static void timbreID_cluster_membership(t_timbreID *x, t_floatarg idx)
{
	...
	else
	{
		...
		listOut = (t_atom *)getbytes(sizeof(t_atom));
...
static void timbreID_compute_order(t_timbreID *x, t_floatarg reference)
{
	...
	// create local memory
	instances = (t_instance *)getbytes(x->num_instances * sizeof(t_instance));
	listOut = (t_atom *)getbytes(x->num_instances * sizeof(t_atom));

	for(i=0; i<x->num_instances; i++)
		instances[i].instance = (float *)getbytes(x->feature_length * sizeof(float));
...
static void timbreID_feature_list(t_timbreID *x, t_floatarg idx)
{
	...
	{
		// create local memory
		listOut = (t_atom *)getbytes(x->feature_length * sizeof(t_atom));
...
static void timbreID_write(t_timbreID *x, t_symbol *s)
{
	...
	// create local memory
	buf = (char *)getbytes(MAXPDSTRING * sizeof(char));

	header = (int *)getbytes((x->num_instances+1) * sizeof(int)); // record the size of each instance's feature, plus 1 for num_instances
...
static void timbreID_read(t_timbreID *x, t_symbol *s)
{
	...
	for(i=0; i<x->num_instances; i++)
		x->instances[i].instance = (float *)getbytes(x->instance_feature_lengths[i] * sizeof(float));
...
static void timbreID_write_text(t_timbreID *x, t_symbol *s)
{
	...
	header = (int *)getbytes(2 * sizeof(int));
...
static void timbreID_read_clusters_text(t_timbreID *x, t_symbol *s)
{
    ...
    for(i=0; i<x->num_instr; i++)
	{
		...
		x->cluster_members[i].member = (int *)getbytes(x->cluster_member_lengths[i]*sizeof(int));
	}
...
static void timbreID_free(t_timbreID *x)
{
	...
	t_freebytes(x->weights, x->feature_length*sizeof(float));
	t_freebytes(x->attribute_order, x->feature_length*sizeof(int));
	t_freebytes(x->instance_feature_lengths, x->num_instances*sizeof(int));
	if(x->normalize)
		t_freebytes(x->norm_data, x->feature_length*sizeof(t_norm_data));
}
