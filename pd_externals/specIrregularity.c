...
static void sIrreg_blackmanWindow(t_float *wptr, int n)
{
	int i;
...
static void sIrreg_cosineWindow(t_float *wptr, int n)
{
	int i;
...
static void sIrreg_hammingWindow(t_float *wptr, int n)
{
	int i;
...
static void sIrreg_hannWindow(t_float *wptr, int n)
{
	int i;
...
int sIrreg_signum(t_sample sample)
{
	int sign, crossings;
...
t_float sIrreg_zeroCrossingRate(int n, t_sample *input)
{
	int i;
	t_float crossings;
	...
	for(i=1; i<n; i++)
		crossings += fabs(sIrreg_signum(input[i]) - sIrreg_signum(input[i-1]));
...
void sIrreg_realfftUnpack(int n, int nHalf, t_sample *input, t_sample *imag)
{
	int i, j;
...
void sIrreg_power(int n, t_sample *real, t_sample *imag)
{
	while (n--)
    {
...
void sIrreg_mag(int n, t_sample *power)
{
	while (n--)
	{
	    *power = sqrt(*power);
...
void sIrreg_normal(int n, t_sample *input)
{
	int i;
...
void sIrreg_log(int n, t_sample *spectrum)
{
	while (n--)
    {
...
void sIrreg_realifft(int n, int nHalf, t_sample *real, t_sample *imag)
{
    int i, j;
    t_float nRecip;
...
void sIrreg_cosineTransform(t_float *output, t_sample *input, int numFilters)
{
	int i, k;
...
void sIrreg_filterbankMultiply(t_sample *spectrum, int normalize, int filterAvg, t_filter *filterbank, int numFilters)
{
	int i, j, k;
...
static void specIrregularity_analyze(t_specIrregularity *x, t_floatarg start, t_floatarg n)
{
	...
	if(x->windowFuncSize != lengthSamp)
	{
		...
		sIrreg_blackmanWindow(x->blackman, x->windowFuncSize);
		sIrreg_cosineWindow(x->cosine, x->windowFuncSize);
		sIrreg_hammingWindow(x->hamming, x->windowFuncSize);
		sIrreg_hannWindow(x->hann, x->windowFuncSize);
	}
	...
	sIrreg_realfftUnpack(window, windowHalf, x->signal_R, signal_I);
	sIrreg_power(windowHalf+1, x->signal_R, signal_I);

	// power spectrum sometimes generates lower scores than magnitude. make it optional.
	if(!x->powerSpectrum)
		sIrreg_mag(windowHalf+1, x->signal_R);

	if(x->normalize)
		sIrreg_normal(windowHalf+1, x->signal_R);
